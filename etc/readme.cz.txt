...
Hlavní vlastnosti:
 ...
 - mnoho uivatelskıch nastavení k pøizpùsobení zvykùm uivatele
...
Rùzné:
 - Drag&Drop URL adres
-  jednoduche rozhraní pro programování aplikací (API) pro pøidání dalších slueb jako pluginy
...
MS Windows: c:\Dokumenty a nastavení\Vaše_uivatelské_jméno\Data aplikací\VitySoft\FRD
            a v registru HKEY_CURRENT_USER\Software\JavaSoft\Prefs\vitysoft\frd
Linux: ~/.FRD

NEKOPÍRUJTE novou verzi programu pøes starou.

Pokud je pouit pøepínaè -p (viz dále) je cesta ke konfiguraèním souborùm shodná s adresáøem programu.
...
volby
  ...
  -p (--portable)     konfiguraèní soubory budou uloeny v adresáøi 'config' u programu,
                      všechny cesty (pokud je to moné) budou ukládány relativnì vùèi adresáøi programu
                      - uiteèné napø. pro USB FLASH disky
